...
export class PlaneacionEstrategicaService {
      ...
      async recalcularObjetivoEspecifico(
    ...
    async recalcularObjetivoGeneral(
    idObjetivoGeneral,
  ) {
    const objetivos =
      await prisma.objetivo_especifico.findMany({
        where: {
          id_objetivo_general:
            idObjetivoGeneral,
        },
    ...
    async recalcularCadena(
    idObjetivoEspecifico,
  ) {
        const objetivo =
      await prisma.objetivo_especifico.findUnique({
        where: {
          id_objetivo_especifico:
            idObjetivoEspecifico,
        },
    ...
    if (!objetivo) {
      return;
        ...
        await this.recalcularObjetivoEspecifico(
      idObjetivoEspecifico,
    );

        await this.recalcularObjetivoGeneral(
      objetivo.id_objetivo_general,
    );

        await this.recalcularPerspectiva(
