...
export class ObjetivoEspecificoService {
  constructor(
    model = new ObjetivoEspecificoModel(),
  ) {
    this.model = model;

    this.objetivoGeneralModel =
      new ObjetivoGeneralModel();
  }
  ...
  async create(data) {
    const objetivoGeneral =
      await this.objetivoGeneralModel.findById(
  ...
  async delete(id) {
    ...
    const total =
      await this.model.countTareas(id);
